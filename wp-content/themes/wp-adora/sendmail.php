 ...
 */
		...
		if(trim($_REQUEST['c_name'] == "")){
			$name_error = __('Você esqueceu de preencher o seu nome', 'adora');
			$error = true;
		}else{
		...
		if(trim($_REQUEST['c_email'] === "")){
			$email_error = __('Você esqueceu de preencher o seu e-mail', 'adora');
			$error = true;
		}else if(!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_REQUEST['c_email']))){
			$email_error = __('Formato de e-mail inválido', 'adora');
			$error = true;
		}else{
		...
		if(trim($_REQUEST['c_message'] === "")){
			$message_error = __('Você deve escrever uma mensagem', 'adora');
			$error = true;
		}else{
		...
		if ($email_sent==true && !defined('MOBILE-VERSION')){
			echo __('** Mensagem enviada com sucesso, obrigado!','adora');
		}
?>