...
function is_mobile(){
	$mobile_info = mobile_device_detect();
	$mobile_force = get_option('ermad_mobile_force', 'false');
	$mobile_active = get_option('ermad_mobile_enabled', 'true');
	
	if ($mobile_active=="false"){
		return false;
	}
	
	if ($mobile_force == 'true'){
		return true;
	}
	
	if (is_array($mobile_info)){
		return true;
	}else{
		return false;
	}
