<?php
	...
	function ermark_sc_contact( $atts, $content = null ){
		...
		$src.='<form id="contact-form" action="'.get_permalink().'" method="post">
					...
					<div>
						<label>'.__('Website (opcional)','adora').'</label>
						<div class="input"><input type="text" name="c_website" id="c_website" value=""/></div>
					</div>
					<div>
						<label>'.__('Sua mensagem').'</label>';
						if(defined('MOBILE-SUBMIT') && $mobile && $message_error != '') { $src.='<p class="error">'.$message_error.'</p>';  }
						$src.='<div class="textarea"><textarea name="c_message" id="c_message" rows="11" cols="138" class="require"></textarea></div>
					</div><div>';
					
					if (is_mobile()){
						$src.='<input type="hidden" name="mobile" value="*"/>';
						$src.= '<input type="submit" class="form-button" value="'.__('Enviar mensagem','adora').'" >';
					}else{
						$src.= '<a href="#" class="form-button submit-contact">'.__('Enviar mensagem','adora').'</a>';
					}
	...
	function ermark_sc_map($atts, $content){
		...
		$code_url = 'https://maps.google.com/maps?saddr=-28.288057,-54.29512&hl=en&sll=-28.300451,-54.233665&sspn=0.121821,0.264187&mra=mift&mrsp=0&sz=13&t=m&z=13'.$location.'&amp;zoom='.$zoom.'&amp;size='.$width.'x'.$height.'&amp;sensor=true'.$code_marker;
